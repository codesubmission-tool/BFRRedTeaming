...
def main(args):
    ...
    if args.input_mask is not None:
        random_mask = torch.load(args.input_mask)['random_mask']
    ...
    # swinir_model = net(in_chans=3, img_size=64, window_size=8,
    #                     img_range=1., depths=[6, 6, 6, 6, 6, 6, 6, 6], embed_dim=180, num_heads=[6, 6, 6, 6, 6, 6, 6, 6],
    #                     mlp_ratio=2, upsampler='nearest+conv', resi_connection='1conv',unshuffle=True,unshuffle_scale=8)
    # pretrained_model = torch.load('/home/zelin3/projects/SwinIR/swinir.ckpt')
    # swinir_model.load_state_dict(pretrained_model, strict=True)
    # swinir_model.eval()
    # swinir_model = swinir_model.to(accelerator.device)
    net = ARCH_REGISTRY.get('CodeFormer')(dim_embd=512, codebook_size=1024, n_head=8, n_layers=9, 
                                            connect_list=['32', '64', '128', '256']).to(accelerator.device)
    ...
    for epoch in range(first_epoch, args.max_train_steps):
        ...
        for _ in range(1):
            with accelerator.accumulate(unet), accelerator.accumulate(text_encoder):
                ...
                if  random_v>0.5:
                    SR_pertubed_images = net((pertubed_images.to(accelerator.device)+1)/2,detach_16=False)[0]
                else:
                    SR_pertubed_images = pertubed_images.to(accelerator.device)
                # print(SR_pertubed_images.to(dtype=weight_dtype).dtype)
    
                latents = vae.encode(SR_pertubed_images.to(dtype=weight_dtype)).latent_dist.sample()
                ...
                print(noisy_latents.dtype,timesteps.dtype,encoder_hidden_states.dtype,unet.dtype)
