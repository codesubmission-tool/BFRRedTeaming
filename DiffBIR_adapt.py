    ...
    for epoch in range(first_epoch, args.max_train_steps):
        ...
        for _ in range(1):
            with accelerator.accumulate(unet), accelerator.accumulate(text_encoder):
                ...
                if  random_v>0.5:
                    SR_pertubed_images = (swinir_model((pertubed_images.to(accelerator.device)+1)/2)-0.5)*2
                else:
                    SR_pertubed_images = pertubed_images.to(accelerator.device)
                # else:
                #     SR_pertubed_images = F.interpolate(F.interpolate(pertubed_images.to(accelerator.device),(64,64)),(512,512))
